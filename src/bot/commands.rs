...
const HELP: &str = "<b>Arnheid</b> — your personal second brain.\n\n\
...
<b>Commands</b>\n\
/ask [question] — search your brain + live web (combined automatically)\n\
/ask --here [question] — search only this chat's captures (+ web)\n\
...
const START_DM: &str = "<b>Welcome to Arnheid</b> 👋\n\n\
...
<code>/ask latest news on topic X</code> — brain + web (augmented automatically)\n\
...
fn start_group(bot_username: &str) -> String {
    format!(
        "<b>Arnheid</b> — your group's ambient memory.\n\n\
         ...
         Web results are combined automatically — no flag needed.\n\n\
...
/// Parse `/ask [--here] [--web-only] question`.
pub fn parse_ask_args(chat_id: i64, args: &str) -> AskRequest {
    let mut scope = QueryScope::Personal;
    let mut web = WebMode::Augment; // web augmentation is always on by default
...
/// Never fails — internal errors are alerted to the admin and turned into a
/// graceful fallback reply by [`query::handler::handle_or_fallback`], so
/// every caller is guaranteed to get something to send back to the chat.
pub async fn run_query(
    ...
    let reply =
        query::handler::handle_or_fallback(state, chat_id, user_id, scope, web, anchor, query_text)
            .await;
    typing.abort();
    reply
}

/// Send a /ask reply. Retries transient delivery failures; alerts the admin
/// if it still can't get the answer out.
pub async fn send_query_reply(state: &AppState, chat_id: i64, reply_to: i32, reply: &QueryReply) {
    let res = send::with_retry(|| async {
        state
            .bot
    ...
    .await;
    if let Err(e) = res {
        tracing::warn!(error = %e, chat_id, "sending query reply failed");
        eprintln!("[warn] sending query reply failed: chat_id={chat_id} error={e}");
        state.metrics.inc_telegram_send_failure();
        alerts::notify(state, "telegram_send_failed", &e.to_string()).await;
    }
}

const COMMAND_FAILED: &str =
    "Something went wrong running that command — try again in a moment. If it keeps happening, check /ping.";

/// If `text` is a command, handle it and return true. The reply is sent here.
/// Every recognized command is guaranteed to produce a reply: an internal
/// failure alerts the admin and sends [`COMMAND_FAILED`] instead of leaving
/// the chat waiting on a response that will never come.
pub async fn try_handle(
    ...
    println!("[info] rx command: command={cmd} has_args={} chat_id={chat_id} user_id={user_id}", !args.is_empty());

    // "ask" manages its own reply and can no longer fail — `run_query`
    // always returns a reply, real or a graceful fallback.
    if cmd == "ask" {
    ...
    let reply = match dispatch(state, chat_id, user_id, &cmd, args, in_private).await {
        Ok(Some(text)) => text,
        Ok(None) => return Ok(false), // unknown command — ignore silently
        Err(e) => {
            tracing::warn!(command = %cmd, error = %e, chat_id, user_id, "command failed");
            eprintln!("[warn] command failed: command={cmd} chat_id={chat_id} user_id={user_id} error={e}");
            alerts::notify(state, &format!("command_failed:{cmd}"), &e.to_string()).await;
            COMMAND_FAILED.to_string()
        }
    };

    send_reply(state, chat_id, reply_to, &reply).await;
    Ok(true)
}

/// Runs every command except `ask`. `Ok(None)` means `cmd` isn't recognized.
async fn dispatch(
    state: &AppState,
    chat_id: i64,
    user_id: i64,
    ...
    Ok(Some(match cmd {
        ...
        _ => return Ok(None), // unknown command — ignore silently
    }))
}

async fn send_reply(state: &AppState, chat_id: i64, reply_to: i32, text: &str) {
    let res = send::with_retry(|| async {
        state
            .bot
    ...
    .await;
    if let Err(e) = res {
        tracing::warn!(error = %e, chat_id, "sending command reply failed");
        eprintln!("[warn] sending command reply failed: chat_id={chat_id} error={e}");
        state.metrics.inc_telegram_send_failure();
        alerts::notify(state, "telegram_send_failed", &e.to_string()).await;
    }
}

/// Timeout for the /ping chat-model canary call.
const PING_CHAT_TIMEOUT: Duration = Duration::from_secs(20);
/// Timeout for the /ping embedding canary call.
const PING_EMBED_TIMEOUT: Duration = Duration::from_secs(10);
/// Timeout for the /ping DB canary query.
const PING_DB_TIMEOUT: Duration = Duration::from_secs(5);

/// Health check: DB, chat model, embeddings, personal captures, taste
/// profile, and channel capabilities. The DB/chat/embedding lines are live
/// canary calls, not just "is a key configured" — this is the diagnostic you
/// reach for when the bot has gone quiet, so it needs to actually catch an
/// unreachable Ollama, an invalid Anthropic key, or a dead DB connection.
async fn ping(state: &AppState, user_id: i64) -> String {
    let (db_line, chat_line, embed_line) =
        tokio::join!(ping_db(state), ping_chat(state), ping_embed(state),);
    ...
    let web = if state.web_search.is_some() {
        "🟢 on (default, use --web-only for web-only)"
    } else {
        "⚪ off (set WEB_SEARCH_CMD)"
    };

    let (db_line, db_ok) = db_line;
    let (chat_line, chat_ok) = chat_line;
    let (embed_line, embed_ok) = embed_line;
    let header = if db_ok && chat_ok && embed_ok {
        "🟢 <b>Arnheid is up.</b>"
    } else {
        "🔴 <b>Arnheid is degraded</b> — see the failing check below."
    };
...
/// Live DB canary: a trivial query with a bounded timeout, not just "is the
/// pool configured."
async fn ping_db(state: &AppState) -> (String, bool) {
    match timeout(
        PING_DB_TIMEOUT,
    ...
    .await
    {
        Ok(Ok(_)) => ("DB: 🟢 ok".to_string(), true),
        Ok(Err(e)) => (
            format!("DB: 🔴 error ({})", esc(&alerts::clip(&e.to_string(), 200))),
            false,
        ),
        Err(_) => (
            format!("DB: 🔴 timeout (>{}s)", PING_DB_TIMEOUT.as_secs()),
            false,
        ),
    }
}

/// Live chat-model canary: exercises the exact Tier 5 (RAG) path `/ask` uses,
/// so a misconfigured provider, an unreachable Ollama, or an invalid
/// Anthropic key shows up here instead of only surfacing as a silent `/ask`
/// failure later.
async fn ping_chat(state: &AppState) -> (String, bool) {
    let probe = state
        .chat
        .synthesize("Reply with exactly the word OK and nothing else.", "ping");
    match timeout(PING_CHAT_TIMEOUT, probe).await {
        Ok(Ok(_)) => ("Chat model: 🟢 ok".to_string(), true),
        Ok(Err(e)) => (
            format!(
                "Chat model: 🔴 error ({})",
                esc(&alerts::clip(&e.to_string(), 200))
            ),
            false,
        ),
        Err(_) => (
            format!("Chat model: 🔴 timeout (>{}s)", PING_CHAT_TIMEOUT.as_secs()),
            false,
        ),
    }
}

/// Live embedding canary, same rationale as [`ping_chat`].
async fn ping_embed(state: &AppState) -> (String, bool) {
    match timeout(PING_EMBED_TIMEOUT, state.embedder.embed("ping")).await {
        Ok(Ok(_)) => ("Embeddings: 🟢 ok".to_string(), true),
        Ok(Err(e)) => (
            format!(
                "Embeddings: 🔴 error ({})",
                esc(&alerts::clip(&e.to_string(), 200))
        ...
        Err(_) => (
            format!(
                "Embeddings: 🔴 timeout (>{}s)",
                PING_EMBED_TIMEOUT.as_secs()
            ),
            false,
        ),
    }
...
async fn reindex(state: &AppState, chat_id: i64) -> Result<String> {
    ...
    for it in missing {
            ...
            .await
        {
            ...
            Ok(_) => {}
            Err(e) => {
                tracing::warn!(item_id = %it.id, error = %e, "reindex failed");
                eprintln!("[warn] reindex failed: item_id={} error={e}", it.id);
            }
        }
    }
