...
async fn register_commands(bot: &Bot) {
    let commands = vec![
        ...
        BotCommand {
            command: "buildgraph".into(),
            description: "Build the knowledge graph now".into(),
        },
        BotCommand {
            command: "ping".into(),
            description: "Check database and bot status".into(),
        },
        BotCommand {
            command: "reindex".into(),
            description: "Re-index existing content chunks".into(),
        },
        BotCommand {
            command: "help".into(),
    ...
    if let Err(e) = bot.set_my_commands(commands).await {
        ...
        eprintln!("[warn] set_my_commands failed: error={e}");
    }
}
