...
pub async fn handle_message(_bot: Bot, msg: Message, state: AppState) -> ResponseResult<()> {
    if let Err(e) = handle_inner(&state, &msg).await {
        ...
        eprintln!("[warn] message handler error: error={e}");
        // Anything reaching here happened before a reply-shaped code path
        // (query/command) had a chance to handle its own error — e.g. a DB
        // write failing before we know what to answer. Rare, but still worth
        // a page: today's silence is exactly this branch going unnoticed.
        alerts::notify(&state, "handler_error", &e.to_string()).await;
    }
    Ok(())
}
...
async fn handle_inner(state: &AppState, msg: &Message) -> anyhow::Result<()> {
    ...
    println!(
        "[debug] rx message: chat_id={chat_id} user={:?} message_id={message_id} len={}",
        username.as_deref().or(first_name.as_deref()),
        text.chars().count()
    );
    ...
    if !urls.is_empty() {
        ...
        // Check if this is ALSO an active query question.
        let body = mentioned_text.as_deref().unwrap_or(text).trim();
        let req = commands::parse_ask_args(chat_id, body);
        let wants_query = capture_eligible
            && (matches!(req.scope, query::QueryScope::Group(_))
                || intake::wants_agent(state, &req.question).await);

        if wants_query {
            // Let the execution drop down into Step 4 to query using this URL as context!
        } else {
            if queued {
                capture_ack(state, msg, "✓ saved — reading it in the background").await;
            }
    ...
    let req = commands::parse_ask_args(chat_id, body);
    let wants_query = matches!(req.scope, query::QueryScope::Group(_))
        || intake::wants_agent(state, &req.question).await;
    ...
    if wants_query {
        ...
        println!("[info] telegram query: chat_id={chat_id} user={uid}");
        ...
        println!("[info] telegram note queued: chat_id={chat_id} user={uid}");
...
async fn handle_voice(
    ...
    println!("[info] telegram voice queued: chat_id={chat_id} user={uid}");
    intake::schedule_note(
        state,
        chat_id,
        msg.chat.title().map(|s| s.to_string()),
...
async fn handle_photo(
    ...
    println!("[info] telegram photo queued: chat_id={chat_id} user={uid}");
    intake::schedule_note(
        state,
        chat_id,
        msg.chat.title().map(|s| s.to_string()),
...
/// least one link was queued. A single URL failing to enqueue (a DB hiccup)
/// is alerted and skipped rather than aborting the rest of a multi-link
/// message — each URL is an independent capture.
async fn schedule_urls(
    ...
    for url in urls {
            ...
            .unwrap_or(false)
        {
            ...
            println!("[info] link shared (duplicate — skipped): url={url} chat_id={chat_id}");
            continue;
        }
        tracing::info!(%url, chat_id, user = uid, "link shared — queued for ingestion");
        println!("[info] link shared — queued for ingestion: url={url} chat_id={chat_id} user={uid}");
        if let Err(e) = intake::schedule_link(
            state,
            url.clone(),
            chat_id,
            msg.chat.title().map(|s| s.to_string()),
        ...
        .await
        {
            tracing::warn!(%url, chat_id, error = %e, "failed to enqueue link");
            eprintln!("[warn] failed to enqueue link: url={url} chat_id={chat_id} error={e}");
            alerts::notify(state, "ingestion_enqueue_failed", &e.to_string()).await;
            continue;
        }
        queued = true;
    }
...
async fn capture_ack(state: &AppState, msg: &Message, text: &str) {
        ...
        .await
    {
        ...
        eprintln!("[warn] capture ack failed: error={e}");
    }
}
...
mod tests {
    ...
    fn telegram_links_skipped() {
        assert!(is_telegram_link("https://t.me/arnheid/123"));
