//! Query handler: embed → vector search → Tier 5 synthesis → formatted reply.
...
const SYSTEM_PROMPT_PERSONAL: &str = "You are Arnheid — a sharp research analyst for \
...
const SYSTEM_PROMPT_GROUP: &str = "You are Arnheid — a sharp research analyst for \
...
const SYSTEM_PROMPT_DUAL: &str = "You are Arnheid — a sharp research analyst. You have \
...
- When a source already contains the concrete answer (a number, date, name, current \
status), lead with that fact directly — never deflect to 'check the link' or describe \
what a page covers instead of answering. If nothing in EXTERNAL actually contains the \
live data the question needs (e.g. current weather, a live price), say plainly that you \
don't have current data for it rather than writing around the gap.\n\
...
const WEB_UNAVAILABLE: &str = "Web search isn't configured — add <code>TAVILY_API_KEY</code> to enable web search.";

const CHITCHAT_SYSTEM: &str = "You are Arnheid, a sharp, warm second-brain assistant talking to \
the user in a chat app. They just sent a greeting or small talk, not a real question — reply \
naturally and briefly (1-2 short sentences), plain text only, no markdown, no citations. Don't \
mention tools, search, or your capabilities unless they specifically asked what you can do.";
...
const QUERY_FAILED: &str =
    "Something went wrong answering that — try again in a moment. If it keeps happening, check /ping.";
...
pub async fn handle(
    ...
    let (anchor_items, focused) = match resolved {
        ...
        AnchorResolve::Partial { url, message_id } => {
            match build_partial_item(state, chat_id, &url, message_id).await {
                Some(item) => {
                    ...
                    println!("[info] ask partial in-flight ingest: chat_id={chat_id} user_id={user_id} url={url}");
                    partial_ingest = true;
                    (vec![item], true)
                }
                None => {
                    tracing::info!(chat_id, user_id, url = %url, "ask pending ingest — no context yet");
                    println!("[info] ask pending ingest — no context yet: chat_id={chat_id} user_id={user_id} url={url}");
        ...
        AnchorResolve::None if deictic => {
            ...
            println!("[info] ask deictic without referent: chat_id={chat_id} user_id={user_id}");
            let reply = QueryReply {
        ...
        AnchorResolve::None if follow_up && !thread_state.active_item_ids.is_empty() => {
            ...
            if items.is_empty() {
                ...
                println!(
                    "[info] ask follow-up inheriting session referents: chat_id={chat_id} user_id={user_id} items={}",
                    items.len()
                );
                (items, true)
            }
    ...
    if focused {
        ...
        println!(
            "[info] ask anchored: chat_id={chat_id} user_id={user_id} item={:?} reply={:?}",
            anchor_item.as_ref().map(|i| i.id),
            anchor.reply_message_id
        );
    }

    let intent = if web == WebMode::Augment && state.web_search.is_some() && !focused {
        match state.chat.classify_query_intent(query_text).await {
            Ok(i) => i,
            Err(e) => {
                tracing::warn!(user_id, error = %e, "query intent classification failed; using default");
                eprintln!("[warn] query intent classification failed; using default: user_id={user_id} error={e}");
                QueryIntent::default()
            }
        }
    } else {
    ...
    .await
    {
        tracing::warn!(user_id, error = %e, "logging query event failed");
        eprintln!("[warn] logging query event failed: user_id={user_id} error={e}");
    }
    ...
    .await
    {
        tracing::warn!(user_id, error = %e, "taste signal update failed after query");
        eprintln!("[warn] taste signal update failed after query: user_id={user_id} error={e}");
    }
    ...
    println!(
        "[info] ask expand: chat_id={chat_id} user_id={user_id} scope={} probes={} query={}",
        scope_label(scope),
        extra_probes.len(),
        snippet(query_text, 80)
    );
    ...
    for attempt in 0..2u8 {
        ...
        println!(
            "[info] ask context: chat_id={chat_id} attempt={attempt} sources={} graph_added={graph_added}",
            sources.len()
        );
        ...
        match resolve_outcome(&raw, items.len()) {
            AnswerOutcome::Answered {
                    ...
                    && needs_external
                {
                    ...
                    println!("[info] ask web augment: chat_id={chat_id} web_queries={}", queries.len());
                ...
                println!(
                    "[info] ask answered: chat_id={chat_id} attempt={attempt} answered=true cited={}",
                    cited.len()
                );
            ...
            AnswerOutcome::NeedMore {
                ...
                if attempt >= 1 || follow_up.is_empty() {
                    ...
                    println!("[info] ask answered: chat_id={chat_id} attempt={attempt} answered=false");
                    return Ok(QueryReply {
                        html: no_results.to_string(),
                        cited_item_ids: vec![],
                    });
                ...
                println!("[info] ask self-correcting: chat_id={chat_id} follow_ups={}", follow_up.len());
                for q in &follow_up {
...
/// Like [`handle`], but never propagates an error to the caller: every
/// channel (Telegram `/ask`/@mention, WhatsApp questions) should always send
/// *something* back, never go silent because an LLM call, an embedding call,
/// or a DB query happened to fail. On error, alerts the admin (rate-limited
/// by [`crate::bot::alerts`]) and returns a graceful fallback reply instead.
///
/// Eligible queries (see [`agentic_eligible`]) run `query::agent`'s loop
/// first — it decides per question whether any tool is needed at all. A
/// failure there falls back to the fixed pipeline below rather than
/// surfacing as an error, so a bad agent turn never means a broken `/ask`.
pub async fn handle_or_fallback(
    state: &AppState,
    chat_id: i64,
    user_id: i64,
    scope: QueryScope,
    ...
    // Greetings and small talk never need brain/web retrieval — decide that
    // deterministically instead of trusting a tool-calling model (esp. a
    // small/fast one) to always follow the "don't search chit-chat" policy
    // in its system prompt. Also skips session/anchor DB round-trips this
    // has no use for.
    if crate::intake::is_chitchat_query(query_text) {
        let reply = chitchat_reply(state, query_text).await;
        state
            .metrics
            .record_ask(crate::metrics::AskOutcome::Answered, start.elapsed());
        return reply;
    }

    match crate::query::agent::handle(state, chat_id, user_id, scope, web, &anchor, query_text).await {
        Ok((reply, outcome)) => {
            state.metrics.record_ask(outcome, start.elapsed());
            reply
        }
        Err(e) => {
            tracing::error!(
                chat_id, user_id, error = %e,
                "agentic loop execution failed"
            );
            state.metrics.record_ask(crate::metrics::AskOutcome::Error, start.elapsed());
            QueryReply {
                html: "Something went wrong in the agentic loop — please try again in a moment.".to_string(),
                cited_item_ids: vec![],
            }
        }
    }
...
/// Heuristic, not exact: a real answer almost always cites at least one
/// source, so an empty cited list is a decent proxy for "nothing relevant
/// found" without threading a dedicated outcome flag through every return
/// point in `handle`. The agentic path doesn't use this — it reports its own
/// outcome, because integration answers legitimately cite nothing.
fn ask_outcome(reply: &QueryReply) -> crate::metrics::AskOutcome {
    if reply.cited_item_ids.is_empty() {
...
/// Direct, tool-free reply to a greeting or small talk — no brain search, no
/// web search, no session bookkeeping. Falls back to a canned reply if the
/// model call itself fails, since even chit-chat must never go silent.
async fn chitchat_reply(state: &AppState, query_text: &str) -> QueryReply {
    let text = state
        .chat
        .synthesize(CHITCHAT_SYSTEM, query_text)
        .await
        .unwrap_or_else(|e| {
            tracing::debug!(error = %e, "chitchat reply failed, using canned fallback");
            println!("[debug] chitchat reply failed, using canned fallback: error={e}");
            "Hey! What can I help you find?".to_string()
        });
    QueryReply {
        html: formatter::query_answer(&text, &[], &[], &[], &[]),
        cited_item_ids: vec![],
    }
}

async fn handle_web_only(state: &AppState, query_text: &str) -> Result<QueryReply> {
    ...
    let snippets = match search.search(query_text).await {
        Ok(s) => s,
        Err(e) => {
            tracing::warn!(error = %e, "web search failed");
            eprintln!("[warn] web search failed: error={e}");
            Vec::new()
        }
    };
...
async fn synthesize_dual_reply(
    ...
    tracing::info!(raw_response = %raw, "raw LLM response for dual synthesis");
    println!("[info] raw LLM response for dual synthesis: raw_response={raw}");
    let DualOutcome {
        answer,
        cited,
        web_cited,
...
/// `pub(crate)`: shared with `query::agent` — see `sanitize_cited`.
pub(crate) fn cited_web_in_text(text: &str, n: usize) -> Vec<usize> {
    let mut out = Vec::new();
...
async fn resolve_anchor(
    ...
    if crate::intake::is_deictic_query(query_text) {
        ...
        .await
        {
            // Most-recent-first order from the query. Walk all candidates
            // looking for one already ingested; only fall back to a Partial
            // placeholder — and then for the most recent URL, not whichever
            // happened to be checked last — if none of them are ready yet.
            ...
            for (mid, url) in urls {
                ...
                if fallback.is_none() {
                    fallback = Some((url, mid));
                }
            }
            if let Some((url, message_id)) = fallback {
...
async fn record_session_turn(
    ...
    .await
    {
        ...
        println!("[debug] session turn persist failed: error={e}");
    }
}
    ...
    match scope {
        QueryScope::Personal => {
            ...
            .await
            {
                Ok(k) => lists.push(k),
                Err(e) => {
                    tracing::debug!(error = %e, "keyword search failed");
                    println!("[debug] keyword search failed: error={e}");
                }
            }
        }
        QueryScope::Group(group_id) => {
...
/// `pub(crate)`: also used by `query::agent`'s final-answer parsing, which
/// follows the exact same `[n]` citation convention.
pub(crate) fn sanitize_cited(sources: &[usize], n: usize) -> Vec<usize> {
...
/// `pub(crate)`: shared with `query::agent` — see `sanitize_cited`.
pub(crate) fn cited_in_text(text: &str, n: usize) -> Vec<usize> {
    let mut out = Vec::new();
    let mut rest = text;
...
async fn expand_with_graph(
    ...
    match db::graph::related_entities(&state.pool, group_id, &seeds, 12).await {
...
/// `pub(crate)`: shared with `query::agent`'s `search_brain` tool.
pub(crate) fn rrf_fuse(lists: Vec<Vec<crate::db::chunks::ChunkHit>>, limit: usize) -> Vec<crate::db::chunks::ChunkHit> {
