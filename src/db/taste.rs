...
fn row_to_profile(
    ...
    UserTasteProfile {
        user_id,
        interest_vector: vec.map(|v| vector::decode(&v)),
...
pub async fn get(pool: &PgPool, user_id: i64) -> Result<Option<UserTasteProfile>> {
    let row: Option<(
        ...
        r#"
        SELECT interest_vector::text, vector_weight, notify_threshold,
...
pub async fn list_active_in_group(
    ...
    let rows: Vec<(
        ...
        r#"
        SELECT t.user_id, t.interest_vector::text, t.vector_weight, t.notify_threshold,
...
pub async fn apply_signal(
    ...
    let vec = vector::encode(&merged_vec);
...
pub async fn stats_for_user(pool: &PgPool, user_id: i64) -> Result<(i64, i64, i64)> {
...
pub async fn calibrate_threshold(
    ...
    tracing::debug!(
        ...
        old = current,
        new = new_threshold,
        "raised notify threshold"
    );
    println!(
        "[debug] raised notify threshold: user_id={user_id} dismissals={dismissals} old={current} new={new_threshold}"
    );
