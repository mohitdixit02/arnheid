...
#[derive(Debug, Clone)]
pub struct ChatMessageHit {
    pub username: Option<String>,
    pub text: String,
    pub created_at: chrono::DateTime<chrono::Utc>,
}

/// Search past chat logs in the message buffer for a specific group or DM chat.
pub async fn search_buffer(
    pool: &PgPool,
    group_id: i64,
    query: &str,
    limit: i64,
) -> Result<Vec<ChatMessageHit>> {
    let rows: Vec<(Option<String>, Option<String>, chrono::DateTime<chrono::Utc>)> = sqlx::query_as(
        r#"
        WITH q AS (
        ...
        WHERE group_id = $1
          ...
          AND to_tsvector('english', text) @@ (SELECT tsq FROM q)
    ...
    Ok(rows
        .into_iter()
        .map(|(username, text, created_at)| ChatMessageHit {
            username,
            text: text.unwrap_or_default(),
            created_at,
        })
