//! Item persistence: insert, dedup, vector search, retry queue.
...
pub async fn insert(pool: &PgPool, item: NewItem<'_>) -> Result<Option<Uuid>> {
    let embedding = item.embedding.map(vector::encode);
...
pub async fn update_after_fetch(
    ...
    let emb = embedding.map(vector::encode);
    sqlx::query(
        r#"
        UPDATE items
...
pub async fn signal_source(pool: &PgPool, id: Uuid) -> Result<Option<(Vec<f32>, Vec<String>)>> {
    let row: Option<(Option<String>, Vec<String>)> = sqlx::query_as(
        "SELECT embedding::text, tags FROM items WHERE id = $1",
    )
    .bind(id)
    .fetch_optional(pool)
    .await?;
    Ok(row.map(|(emb, tags)| (emb.map(|v| vector::decode(&v)).unwrap_or_default(), tags)))
}
