...
pub async fn replace_for_item(
    ...
    for (idx, content, embedding) in rows {
        let vec = vector::encode(embedding);
        sqlx::query(
            r#"
...
pub async fn keyword_search_by_owner(
    ...
    let result: std::result::Result<Vec<ChunkRow>, sqlx::Error> = sqlx::query_as(
        r#"
        WITH q AS (
        ...
        SELECT c.id, c.item_id, i.url, i.title, i.summary, i.shared_by, u.username,
               i.message_id, i.shared_at, c.content,
               i.source_channel, i.content_type, i.group_id,
               ts_rank(to_tsvector('english', c.content), (SELECT tsq FROM q))::float8 AS similarity
        FROM chunks c
        JOIN items i ON i.id = c.item_id
        LEFT JOIN users u ON u.id = i.shared_by
    ...
    match result {
        ...
        Err(e) if is_lexeme_less_query_error(&e) => Ok(Vec::new()),
        Err(e) => Err(e.into()),
    }
}
...
/// CockroachDB's tsquery parser (unlike Postgres) hard-errors instead of
/// returning an empty tsquery when the input reduces to zero lexemes (e.g.
/// all-stopword search terms) — treat that as "no matches", which is what
/// the empty-tsquery guard would have meant on Postgres.
fn is_lexeme_less_query_error(e: &sqlx::Error) -> bool {
    e.as_database_error()
        .is_some_and(|d| d.message().contains("doesn't contain lexemes"))
}
