...
pub async fn get(pool: &PgPool, user_id: i64, group_id: i64) -> Result<Option<UserProfile>> {
    let row: Option<(Option<String>, f64, f64, Vec<String>, Option<DateTime<Utc>>)> =
        sqlx::query_as(
            r#"
            SELECT interest_vector::text, vector_weight, relevance_threshold, top_tags, muted_until
...
pub async fn list_active_in_group(
    ...
    let rows: Vec<(i64, Option<String>, f64, f64, Vec<String>, Option<DateTime<Utc>>)> =
        sqlx::query_as(
            r#"
            SELECT user_id, interest_vector::text, vector_weight, relevance_threshold, top_tags, muted_until
            FROM user_profiles
