...
pub async fn process_message(state: AppState, msg: Message, profile_name: Option<String>) {
    if let Err(e) = process_inner(&state, &msg, profile_name).await {
        ...
        eprintln!("[warn] whatsapp message failed: msg_id={} error={e}", msg.id);
        crate::bot::alerts::notify(&state, "whatsapp_handler_failed", &e.to_string()).await;
    }
}
...
async fn process_inner(state: &AppState, msg: &Message, profile_name: Option<String>) -> Result<()> {
    ...
    if !db::channels::record_event(&state.pool, CHANNEL, &msg.id).await? {
        ...
        println!("[debug] duplicate delivery ignored: msg_id={}", msg.id);
        return Ok(());
    }
    ...
    if let Err(e) = wa.mark_read(&msg.id).await {
        ...
        println!("[debug] mark_read failed: error={e}");
    }
    ...
    match msg.kind.as_str() {
        "text" => {
            ...
            if !urls.is_empty() {
                ...
                for url in urls {
                    ...
                    .unwrap_or(false)
                    {
                        ...
                        println!("[info] link shared (duplicate — skipped): url={url} group_id={group_id}");
                        continue;
                    }
                    tracing::info!(%url, group_id, user = user_id, "whatsapp link queued");
                    println!("[info] whatsapp link queued: url={url} group_id={group_id} user={user_id}");
                    ...
                    .await
                    {
                        tracing::warn!(%url, group_id, error = %e, "failed to enqueue link");
                        eprintln!("[warn] failed to enqueue link: url={url} group_id={group_id} error={e}");
                        crate::bot::alerts::notify(
                            state,
            ...
            } else if intake::wants_agent(state, text).await {
                tracing::info!(group_id, user = user_id, "whatsapp query");
                println!("[info] whatsapp query: group_id={group_id} user={user_id}");
                let req = crate::bot::commands::parse_ask_args(group_id, text);
                // Never fails silently: internal errors alert the admin and
                // become a graceful fallback reply instead of no reply at all.
                let reply = query::handler::handle_or_fallback(
                    state,
                    group_id,
                    user_id,
                ...
                .await;
                if let Err(e) = wa
                    .send_text(&msg.from, &format::html_to_plain(&reply.html))
                    .await
                {
                    tracing::warn!(error = %e, group_id, "whatsapp reply delivery failed");
                    eprintln!("[warn] whatsapp reply delivery failed: group_id={group_id} error={e}");
                    crate::bot::alerts::notify(state, "whatsapp_send_failed", &e.to_string()).await;
                }
            } else {
        ...
        other => {
            ...
            println!("[info] unsupported whatsapp message type: kind={other}");
