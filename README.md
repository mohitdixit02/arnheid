<h1 align="center">Arnheid</h1>

<p align="center">
  <b>A context capture protocol.</b><br>
  Forward anything — links, notes, voice notes, photos — from any connected chat channel.
  Arnheid builds a private knowledge brain over it and answers questions like a researcher,
...
Social media is where you find things — and where you lose them. Arnheid is the
proper home for everything you meant to keep: forward it to the Arnheid bot on
...
- 🔎 **Hybrid retrieval** — semantic (CockroachDB's distributed vector index) **+** keyword (full-text search), fused with Reciprocal Rank Fusion.
...
- 🪙 **Cheap & swappable models** — hosted Hugging Face embeddings + any OpenAI-compatible chat endpoint (DeepSeek, Groq, local, or Claude), routable per tier.
- 🦀 **One Rust binary** — bot, ingestion worker, scorer, query engine, and cron all in-process. CockroachDB alongside as the persistent memory layer.
- 🔁 **Durable ingestion queue** — every capture is a row from the moment it's shared; a restart never loses one, and failures retry with backoff before giving up.
- 📈 **`/metrics`** — Prometheus text exposition for ask outcomes/latency, ingestion throughput, queue depth, and send/notification counters.
...
Telegram update            WhatsApp webhook (signed POST)
 ...
 ingestion worker (continuous):
   fetch (article | youtube | pre-extracted) → extract → summarize+tag (LLM)
     → chunk into passages → embed each (hosted) → store
...
| 3    | embed passages, queries, interest vectors                    | Hugging Face `BAAI/bge-large-en-v1.5` (1024-d) |
...
5. **Self-correction** — if the model reports it can't answer, it returns
   `follow_up` queries; Arnheid retrieves again targeting the gap and synthesizes
...
`RAG_MODE=agentic` (the default) replaces the fixed sequence above with a
bounded loop where the model decides, every turn, whether it already has
what it needs:

1. **Decide** — small talk, "what can you do", and anything answerable
   outright get answered on turn one with no tools and no searches. Only a
   question that actually needs the user's material, the live web, or a
   connected account triggers a call.
2. **Call** — it picks from one list: `search_brain`, `graph_lookup`,
   `web_search`, and every MCP integration (`gsuite_gmail_search`, …). Up to
   5 turns and 3 calls per turn; repeats of an identical call are refused.
3. **Answer** — when it sets `final_response: true`, that turn's message is
   what the user sees.

Each turn is a JSON object — `{reasoning, message, tool_calls,
final_response}` — parsed out of the model's text rather than
provider-native tool calling, so the loop runs on Groq/DeepSeek/Ollama as
well as Claude. The reasoning and every tool call are logged (`agent step`,
one line per turn) and never sent to the user; only the final message is.

Anchored ("about this link"), deictic, and follow-up queries stay on the
fixed pipeline regardless of `RAG_MODE`, since they need session continuity
the agentic path doesn't have yet. If the loop errors or the model can't
produce valid JSON twice, that single query falls back to the fixed pipeline
rather than failing — `arnheid_agentic_fallback_total` in `/metrics` tracks
how often that happens.

### Integrations: the universal MCP layer

Every tool the agent can call — the built-in `search_brain`, `graph_lookup`
and `web_search` included — is described and dispatched through one surface
(`src/mcp/toolbox.rs`), so the loop never special-cases a backend. On top of
the built-ins, `src/mcp/` registers third-party tools from two kinds of
backend behind the same namespaced list:

- **Remote** — any MCP server reachable over Streamable HTTP, configured as
  `MCP_SERVERS=linear=https://mcp.linear.app/mcp,…`, with each server's bearer
  token in `MCP_TOKEN_<SLUG>`. Tools are listed once at boot and exposed as
  `linear_create_issue`, `linear_search`, …
- **Built-in GSuite** — Gmail, Calendar and Drive, implemented in-process
  against Google's REST APIs, so there's no sidecar to deploy and the Google
  refresh token never leaves the bot: `gsuite_gmail_search`, `gsuite_gmail_read`,
  `gsuite_calendar_events`, `gsuite_drive_search`. Read-only. Set it up with

  ```bash
  python3 scripts/google_oauth_consent.py ~/Downloads/client_secret_*.json  # prints the 3 GOOGLE_* vars
  ```

So `/ask what did the Acme contract email say, and how does their pricing
compare to what's on their site?` can search Gmail, search the web, and answer
in one turn — the same loop, the same reply, the same channel.

Unlike `web_search`, integrations aren't gated on `--web`: the closed-corpus
default is about not leaking the question to a search engine, which doesn't
apply to the user's own accounts. Integration output is context, not a
numbered source — `[n]`/`[Wn]` citations stay owned by the brain and web
tools.

Everything a remote server says is untrusted input to a prompt: tool
descriptions and results are stripped of control characters and truncated at
the boundary, tool names are namespaced by slug, and no server may claim a
name that would shadow `search_brain`, `graph_lookup`, or `web_search`. A
server that's down or slow is logged and skipped at boot rather than taking
the bot with it.
...
**1. CockroachDB** — easiest via Docker for local dev:

```bash
docker compose up -d        # CockroachDB on :26257, `arnheid` DB + vector index feature auto-enabled
```

For CockroachDB Cloud instead (needed for AWS deployment), see
[`scripts/provision_cockroachdb.sh`](scripts/provision_cockroachdb.sh) — it
walks the `ccloud` CLI through creating a free Serverless cluster.

**2. Models** — embeddings (Tier 3) are hosted: create a [Hugging Face
token](https://huggingface.co/settings/tokens) with the "Inference Providers"
permission and put it in `HF_API_KEY`. For chat, either install
[Ollama](https://ollama.com) and pull a local model, or point the `OLLAMA_*`
...
```bash
cp .env.example .env
# set TELEGRAM_BOT_TOKEN + HF_API_KEY, and an OLLAMA_API_KEY if using a hosted chat provider
...
2. **Disable privacy mode** — BotFather → `/setprivacy` → **Disable**. Without this, Telegram only delivers commands/mentions and Arnheid can't watch ambient links.
...
The database is CockroachDB — provision it first with
[`scripts/provision_cockroachdb.sh`](scripts/provision_cockroachdb.sh) (Cloud
Serverless, via the `ccloud` CLI). Then `install.sh` installs everything else
— Rust, Ollama + models, `yt-dlp`, the release build, and a systemd service.
It's idempotent.

```bash
# get the code onto the VM, into your home dir (a non-root user can't write /opt):
rsync -av --exclude target --exclude .env --exclude .git ./ user@VM_IP:arnheid/

ssh user@VM_IP
cd ~/arnheid
DATABASE_URL='postgresql://...cockroachlabs.cloud:26257/arnheid?sslmode=verify-full' \
  sudo -E bash install.sh   # → copies to /opt/arnheid, builds, registers service
nano /opt/arnheid/.env        # set TELEGRAM_BOT_TOKEN, HF_API_KEY (+ chat provider key)
sudo systemctl start arnheid
journalctl -u arnheid -f
```

`install.sh` is configurable via env (`HF_API_KEY`, `EMBED_MODEL`, `CHAT_MODEL`,
`INSTALL_CHAT_MODEL`, …) and fills in embedding/chat model settings for you —
you only add the tokens. `DATABASE_URL` is required on first install; re-runs
reuse the one already in `.env` if you don't pass it again.

**Hardware note:** nothing heavy runs on the box — embeddings go to Hugging
Face, and the chat tiers should point at a cheap hosted OSS endpoint
(DeepSeek / Groq) for fast, near-free `/ask`. 64 GB RAM doesn't make CPU
generation fast — a 70B model fits but crawls.
...
| `DATABASE_URL`                                             | —                            | CockroachDB connection string (Postgres wire-compatible)               |
...
| `PORT`                                                     | `8080`                       | HTTP port — always serves `/health` + `/metrics`; also the WhatsApp webhook when that channel is configured (put Caddy in front for TLS) |
...
| `HF_API_KEY`                                               | —                            | required — Hugging Face token ("Inference Providers" scope) for embeddings |
| `EMBEDDING_MODEL` / `EMBEDDING_DIM`                        | `BAAI/bge-large-en-v1.5` / `1024` | HF embedding model + its dimension (must match; set at first boot) |
| `EMBEDDING_BASE_URL`                                       | HF `hf-inference` router     | models root for the feature-extraction API                             |
| `TIER2_PROVIDER` / `GRAPH_PROVIDER` / `RAG_PROVIDER`       | `ollama`                     | per-tier chat backend: `ollama` (any OpenAI-compatible) or `anthropic` |
| `RAG_MODE`                                                 | `agentic`                    | `/ask` retrieval: `agentic` (model decides when to use which tool) or `pipeline` (fixed sequence) |
...
src/
  ...
  mcp/                  universal integration layer: MCP-over-HTTP client, built-in GSuite, tool registry
  db/                   sqlx access: items, chunks, entities, edges, graph, profiles, channels, vector (encode/decode), …
  ...
  cron/                 scheduler + jobs
migrations/             001–013 (schema, vector columns, profiles, buffer, notifications, content_type,
                        chunks, FTS, channels + backfill, user brain + backfill, chat sessions)
install.sh              one-shot VM installer (CockroachDB provisioned separately)
docker-compose.yml      local single-node CockroachDB
scripts/provision_cockroachdb.sh  CockroachDB Cloud cluster via the ccloud CLI
scripts/google_oauth_consent.py   one-time Google consent → GOOGLE_* env vars (stdlib only)
```
...
MIT © Arnheid contributors. See [LICENSE](LICENSE).
